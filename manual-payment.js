    ...
    <div style="margin-top:30px; border:1px solid #ddd; padding:20px; font-family:Arial, sans-serif; border-radius:8px; color:#333; max-width: 500px;">
      ...
      <table style="width:100%; border-collapse:collapse; margin-top:15px; font-size:15px;">
        ...
        <tr style="font-weight:bold; background:#f9f9f9;">
          <td style="padding:10px; font-size:1.1em;">TOTAL PAGADO</td>
...
async function completeManualOrder(req, res) {
  try {
    ...
    const facturaHtmlFull = `<html><body>${buildManualInvoiceHtml(invoiceData)}</body></html>`;
    const htmlBody = `
      <div style="font-family:Arial, sans-serif; color:#333;">
        <p>Su pago ha sido verificado con éxito.</p>
        <p><strong>Su código de licencia:</strong><br>
        <span style="font-size:18px; color:#2a6edb;">${code}</span></p>
        <p>Adjuntamos su factura detallada en formato PDF.</p>
        ${buildManualInvoiceHtml(invoiceData)}
      </div>`;

    const pdfBuffer = await htmlPdf.generatePdf({ content: facturaHtmlFull }, { format: 'A4' });
